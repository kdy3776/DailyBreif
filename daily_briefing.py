...
## 🏦 거시 지표 대시보드
웹서치로 확인한 '현재값'을 아래 표로 정리(미국·한국). 각 행 [지표 | 현재값 | 비고].
예: 미국 CPI, 근원 PCE, 10년물 금리, 연준 기준금리 스탠스, 원/달러, 한국 CPI·기준금리 등.

| 지표 | 현재값 | 비고 |
|---|---|---|
| ... | ... | ... |

그리고 아래 줄에 반드시 다음 토큰만 단독으로 출력하세요(여기에 12개월 추이 그래프가 삽입됩니다):
{{MACRO_CHART}}

## 🏭 산업 섹터 분석
아래 11개 섹터를 '대구분'으로 매일 다루세요. 그날 이슈가 큰 섹터를 위로 정렬하고, 표로 작성:
[섹터 | 모멘텀 | 핵심 이슈(2~3개 이상) | 저평가·과매도 후보(1종)].
- 모멘텀: 🔥과열·▲강세·→중립·▼약세 중 하나.
- 핵심 이슈: 반드시 2~3개 이상을 세미콜론(;)으로 구분해 압축.
- 저평가·과매도 후보: 그 섹터에서 '저평가(밸류)' 또는 '과매도(낙폭과대)'로 볼 수 있는 종목 1개를
  한·미 가리지 말고 고르고, 종목 뒤에 짧은 근거를 괄호로 표기.
  (예: "○○(선행PER 6배·업종 평균 이하)", "△△(52주 최저권·RSI 과매도)")
  ※ 단정적 매수 추천이 아니라 '저평가/과매도로 해석될 수 있는' 후보임을 전제로 하며, 근거는 웹서치로 확인.
...
| 섹터 | 모멘텀 | 핵심 이슈 | 저평가·과매도 후보(1) |
|---|---|---|---|
| ... | ... | ...; ...; ... | ...(근거) |

표 바로 아래에 **"핵심 이슈 근거"** 소제목으로, 위 표의 주요 이슈들이 무엇에 근거했는지
(어떤 발표·실적·지표·뉴스인지) 3~5줄로 요약하세요. 모든 이슈는 웹서치로 확인한 사실에 기반해야 합니다.
...
- **리스크(약세):** ...
- **체크포인트:** ...
- **밸류/과열 진단:** 저평가·과매도 / 촉매 기반 / 분위기·밈성 과열 중 명확히 표시(+근거)

## 🎯 자기계발 한 스푼
(오늘 시장에서 끌어낼 수 있는 투자·사고법 인사이트 한 가지)

---
*본 브리핑은 정보 제공 목적이며 투자 권유가 아닙니다. 투자 판단과 책임은 본인에게 있습니다.*
"""
...
def build_user_prompt() -> str:
    ...
    return (
        f"오늘은 {date_str}입니다. 오늘자 데일리 투자 브리핑을 작성하세요. "
        f"주목 종목은 {STOCK_PICKS}개 범위에서 그날 이슈에 따라 정하세요. "
        f"반드시 웹서치로 최신 정보를 확인한 뒤 작성하고, 시스템 지침의 형식을 지키세요."
    )
